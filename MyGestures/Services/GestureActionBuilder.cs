...
public static class GestureActionBuilder
{
    ...
    {
        if (string.Equals(config.ActionType, GestureConfig.MouseActionType, StringComparison.OrdinalIgnoreCase))
        {
    ...
    private static MoveDirection ToMoveDirection(string value)
    {
        return value switch
        {
            nameof(MoveDirection.Up) => MoveDirection.Up,
            nameof(MoveDirection.Down) => MoveDirection.Down,
            nameof(MoveDirection.Left) => MoveDirection.Left,
            nameof(MoveDirection.Right) => MoveDirection.Right,
