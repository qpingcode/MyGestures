...
public class MouseHook : IMouseHook
{
    ...
    private static readonly TimeSpan HookLifecycleTimeout = TimeSpan.FromSeconds(5);
    ...
    public void StartListening()
    {
        ...
        }) { IsBackground = true, Name = "MyGestures mouse hook" };
        hookThread.SetApartmentState(ApartmentState.STA);
        hookThread.Start();
        if (!started.Wait(HookLifecycleTimeout)) throw new TimeoutException("Mouse hook startup timed out.");
        if (failure != null) throw new InvalidOperationException("Mouse hook startup failed.", failure);
    }
    ...
    public void Dispose()
    {
        ...
        if (hookThread?.IsAlive == true && hookThread != Thread.CurrentThread) hookThread.Join(HookLifecycleTimeout);
        hookThread = null;
        hookDispatcher = null;
    }
}
